  ...
  ws.onmessage = (event) => {
    ...
    if (data.message === 'states' && Array.isArray(data.controls)) {
      for (const ctrl of data.controls) {
        switch (ctrl.name) {
          case 'vu':         vu.value = !!ctrl.state; break;
          case 'white':      white.value = !!ctrl.state; break;
          case 'animation':
            animationEnabled.value = !!ctrl.state;
            if (typeof ctrl.animation === 'number') animationValue.value = ctrl.animation;
            break;
          case 'colour':     if (typeof ctrl.state === 'string') colour.value = ctrl.state; break;
          case 'brightness': if (typeof ctrl.state === 'number') brightness.value = ctrl.state; break;
        }
      }
