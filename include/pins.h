 ...
 * Display module, LEDManager, and the audio task. This wiring is SETTLED: do not
 * change a value without a hardware reason.
 *
 * ONLY GPIO 10, 11, 12, 13, 14 and 21 leave the board (the Extended IO header),
 * plus GPIO0/TXD0/RXD0 on the debug header. Everything else here is soldered to
 * the panel or the touch controller. GPIO10 is the only ADC1 pin of those, which
 * matters because ADC2 is unusable while WiFi is up. The full header pinout is in
 * CLAUDE.md under "Board: WT32-SC01 Plus breakout". (The PlatformIO `board = esp32-s3-devkitc-1` is just a generic S3
