 ...
 * `light` (brightness + rgb + effect) and two `switch`es (VU mode, screen sleep)
 * on connect, so HA auto-creates the entities with NO custom integration or
 * add-on required.
 *
...
private:
    ...
    void onConnected();
    void onDisconnected();
    void onError(const esp_mqtt_error_codes_t* err);
    const char* userForLog() const;   // username, or "(no username)"
    ...
    void handleSleepCommand(const char* payload);
    ...
    int lastFailureKey_ = 0;        // latched failure signature; MQTT event task only
