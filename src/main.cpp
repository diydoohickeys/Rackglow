...
void setup()
{
    ...
    // Initialize network (WiFi + boot screen)
    ...
    if (!g_wifiBootManager->isInSetupMode()) {
        // Boot screen goes FIRST, before the UI allocates. Nothing renders between
        // here and startRenderTask() below — no lv_refr_now, and the render task
        // isn't up yet — so the panel simply holds the last flushed boot frame
        // either way, and freeing first means the UI's objects don't land on top
        // of ~24 boot widgets that are about to leave holes under them. LVGL draws
        // into layers taken from that same fixed pool, so its largest free block
        // is a rendering cost, not just a headroom number.
        g_wifiBootManager->setBootStatus("Starting interface");
        g_wifiBootManager->cleanupBootUI();
        UIManager::logLvglMemory("after boot screen freed");
