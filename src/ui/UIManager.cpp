...
// Render cost, sampled over an interval rather than reported per frame — a
// per-frame line would cost more than the thing it measures and would evict the
// history it exists to preserve.
//
// Most passes do nothing: LVGL only repaints when its refresh timer fires
// (LV_DEF_REFR_PERIOD, 33 ms here), so passes under FRAME_FLOOR_US are not frames
// and are excluded from the mean. `frames` is the positive self-test — if a line
// ever reports 0 it is the instrument that is broken, not the UI that is idle,
// because the line is suppressed entirely when nothing drew.
//
// The bracket covers the whole painting half of update(), not just
// lv_timer_handler: a scene screensaver paints through ScenePlayer and never
// touches LVGL, so timing the handler alone reported a near-idle render task
// while it was in fact driving the entire panel.
//
// bands/frame is the direct evidence that Display::DRAW_BUF_ROWS took effect:
// a full-screen repaint is ceil(480 / rows) bands.
// LED fps rides on the same line, because the number only means anything next to
// what the display was doing at the time: the render task is priority 2 on core 1
// and LEDManager::update() is priority 1 on the SAME core, so the strips are
// paced by whatever the display leaves behind. Comparing a "saver" interval with
// a plain one is the measurement.
constexpr uint32_t FRAME_FLOOR_US = 1000;
constexpr uint32_t REPORT_INTERVAL_MS = 10000;

uint32_t s_frames = 0;
uint32_t s_frameUsTotal = 0;
uint32_t s_frameUsMax = 0;
uint32_t s_lastReportMs = 0;
bool s_saverSeen = false;

void reportFrameTime(uint32_t elapsedUs, bool screensaverUp) {
    if (screensaverUp) s_saverSeen = true;
    if (elapsedUs >= FRAME_FLOOR_US) {
        s_frames++;
        s_frameUsTotal += elapsedUs;
        if (elapsedUs > s_frameUsMax) s_frameUsMax = elapsedUs;
    }

    const uint32_t now = millis();
    if (now - s_lastReportMs < REPORT_INTERVAL_MS) return;
    s_lastReportMs = now;

    const uint32_t bands = Display::takeFlushCount();
    const uint32_t leds = g_ledManager ? g_ledManager->takeShowCount() : 0;
    const uint32_t ledFpsX10 = leds * 10000u / REPORT_INTERVAL_MS;

    // A scene screensaver drives the panel directly and asks LVGL for nothing, so
    // s_frames is legitimately 0 while it runs — which is exactly the interval we
    // most want the LED number from. Report whenever EITHER did work.
    if (s_frames == 0 && leds == 0) {
        s_frameUsMax = 0;   // genuinely idle: no line, and no zeroes to misread
        s_saverSeen = false;
        return;
    }

    if (s_frames == 0) {
        Logger.info("Render: no LVGL frames%s | LEDs %u.%u fps",
                    s_saverSeen ? " (scene saver driving the panel)" : "",
                    (unsigned)(ledFpsX10 / 10), (unsigned)(ledFpsX10 % 10));
    } else {
        Logger.info("Render: %u frames, mean %u.%02u ms, max %u.%02u ms, "
                    "%u.%u bands/frame%s | LEDs %u.%u fps",
                    (unsigned)s_frames,
                    (unsigned)(s_frameUsTotal / s_frames / 1000),
                    (unsigned)((s_frameUsTotal / s_frames / 10) % 100),
                    (unsigned)(s_frameUsMax / 1000),
                    (unsigned)((s_frameUsMax / 10) % 100),
                    (unsigned)(bands / s_frames),
                    (unsigned)((bands * 10 / s_frames) % 10),
                    s_saverSeen ? " [saver]" : "",
                    (unsigned)(ledFpsX10 / 10), (unsigned)(ledFpsX10 % 10));
    }
...
void UIManager::update() {
    ...
    if (otaScreen_.changed()) {
        ...
        if (otaScreen_.isActive()) {
            Display::wake();
        }
        otaScreen_.apply();
    }

    // Everything that PAINTS starts here, and the frame timer brackets all of it.
    // ⚠ A scene screensaver draws through ScenePlayer, not LVGL, so timing
    // lv_timer_handler() alone reports a near-idle render task while it is in fact
    // painting the whole panel. Deliberately left outside: drainUiCommands,
    // serviceWake (blocks 120 ms on a panel wake by design) and the OTA screen.
    ...
    if (audioVisualiser_) {
        if (audioVisualiser_->isActive()) {
            // Live-dismiss if it was disabled from the web while showing, and
            // stop drawing it into a sleeping panel.
            if (!screensaverEnabled_ || Display::isAsleep()) {
                audioVisualiser_->hide();
            } else {
            ...
            // Don't kick in when disabled, asleep, or over the OTA progress screen.
            if (screensaverEnabled_ && !Display::isAsleep() && !otaScreen_.isActive() &&
    ...
    // A screenshot needs one full frame in the mirror, and only this task may call
    // lv_*, so the invalidate that produces it belongs here — immediately before
    // the handler that will service it.
    if (ScreenMirror::takeArmRequest()) {
    ...
    reportFrameTime(micros() - t0,
                    audioVisualiser_ && audioVisualiser_->isActive());
}
...
bool UIManager::createTabview() {
    ...
    // Three tabs, one horizontal swipe apart. Effects has its own page so the
    // fifteen animations are shown as tiles rather than hidden behind a
    // dropdown's scrolling word list.
