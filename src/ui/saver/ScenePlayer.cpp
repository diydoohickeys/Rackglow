...
void ScenePlayer::startCurrentScene() {
    // Covers start(), next() and setCurrent(): a scene swap must never resume a
    // half-finished sweep, or the new scene paints into the old one's bands.
    cursorY_ = 0;
...
void ScenePlayer::update() {
    ...
    // Pacing applies only BETWEEN frames — mid-sweep we always continue, or a
    // slow scene would leave a half-painted screen sitting there.
    if (cursorY_ == 0) {
        ...
        // Sampled once for the whole sweep, and the scene advances exactly once
        // here: per-strip SAMPLING would tear the picture into bands from
        // different moments. Spreading the PAINT across calls does not — the
        // scene state being read is frozen for the whole sweep.
        const AudioFrame frame = g_audioBus.latest();
        scene->advance(frame);
    }

    // One batch, then return, so the render task's own vTaskDelay hands the core
    // to loopTask and the LED update. The bus transaction is opened and closed
    // per batch rather than held across that gap.
    Display::beginDirect();
    for (int32_t i = 0; i < STRIPS_PER_CALL && cursorY_ < height_; ++i) {
        const int32_t rows = (cursorY_ + STRIP_ROWS <= height_) ? STRIP_ROWS
                                                                : (height_ - cursorY_);
        scene->renderStrip(strip_, cursorY_, rows);
        Display::pushStrip(cursorY_, rows, strip_);
        cursorY_ += rows;
    }
    Display::endDirect();

    if (cursorY_ >= height_) cursorY_ = 0;   // sweep complete
}
