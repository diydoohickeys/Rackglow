...
// NVS namespace for broker config.
...
// HA's switch payloads are ON/OFF; accept the common hand-typed variants too.
bool isOnPayload(const char* payload) {
    return strcmp(payload, "ON") == 0 || strcmp(payload, "on") == 0 ||
...
void MqttManager::begin() {
    ...
    objectId_ = String("rackglow_") + shortId_;
...
void MqttManager::loadConfig() {
    ...
    cfg_.prefix = prefs.getString("prefix", "rackglow");
    prefs.end();
}
...
void MqttManager::buildTopics() {
    ...
    topicSleepSet_ = baseTopic_ + "/sleep/set";
    topicSleepState_ = baseTopic_ + "/sleep/state";
...
void MqttManager::applyConfig(bool enabled, const String& host, uint16_t port,
    ...
    prefs.putString("prefix", prefix.length() ? prefix : String("rackglow"));
    prefs.end();
...
void MqttManager::handleInbound(const InboundMsg& msg) {
    ...
    } else if (topicSleepSet_ == msg.topic) {
        handleSleepCommand(msg.payload);
    }
}
...
void MqttManager::handleVuCommand(const char* payload) {
    ...
    cmd.boolValue = isOnPayload(payload);
    g_uiManager->postUiCommand(cmd);
}

void MqttManager::handleSleepCommand(const char* payload) {
    if (!g_uiManager) return;
    UiCommand cmd;
    cmd.type = UiCommandType::SetSleep;
    cmd.boolValue = isOnPayload(payload);
    g_uiManager->postUiCommand(cmd);
}
...
void MqttManager::publishDiscovery() {
    ...
    dev["name"] = "Rackglow";
    dev["manufacturer"] = "DIY Doohickeys";
    dev["model"] = "Rackglow";
    ...
    // Screen sleep switch. ON = backlight + panel off; a screen tap wakes it and
    // the state publish flips this back to OFF.
    {
        JsonDocument doc;
        doc["name"] = "Screen Sleep";
        doc["has_entity_name"] = true;
        doc["unique_id"] = objectId_ + "_sleep";
        doc["command_topic"] = topicSleepSet_;
        doc["state_topic"] = topicSleepState_;
        doc["availability_topic"] = topicStatus_;
        doc["icon"] = "mdi:monitor-off";
        doc["device"] = dev.as<JsonObjectConst>();

        String out;
        serializeJson(doc, out);
        publish("homeassistant/switch/" + objectId_ + "_sleep/config", out, 1, true);
    }
...
void MqttManager::publishStateIfChanged(bool force) {
    ...
    bool asleep = Display::isAsleep();
    bool sleepChanged = force || asleep != lastSleep_;
    ...
    if (sleepChanged) {
        publish(topicSleepState_, asleep ? "ON" : "OFF", 0, true);
        lastSleep_ = asleep;
    }
}
