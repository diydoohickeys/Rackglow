...
void WebApi::registerRoutes(AsyncWebServer* server) {
    ...
    // LED Configuration page — served by the SPA. We send index.html and
    // let the client-side router show the LedConfig view based on
    ...
    server->on("/save-led-config", HTTP_POST, [](AsyncWebServerRequest* request) {
        ...
        if (numStrips > 0 && ledsPerStrip > 0) {
            ...
            // Schedule restart — the web client shows the "Restarting..." view.
    ...
    server->on("/get-settings", HTTP_GET, [](AsyncWebServerRequest* request) {
        ...
        String mqttPrefix = g_mqttManager ? g_mqttManager->prefix() : String("rackglow");
    ...
    // The Vue bundle is linked into the firmware (see include/WebAssets.h), so
